...
└── docs/                        # 文档
    ...
    ├── PROJECT_SUMMARY.md       # 项目总结 (Python 版本)
    └── README_CN.md             # Python 版本中文文档
```
...
3. **搜索技能**
   ...
   python src/index.py search "react"
   ```
...
### search 命令
```bash
python src/index.py search "<搜索关键词>"
```
- 搜索技能数据库
- 显示前 10 个匹配结果
- 输出人类可读格式 + JSON 格式
...
### install 命令
```bash
python src/index.py install "<搜索关键词>" <编号>
```
- 搜索并安装指定编号的技能
- 自动下载 SKILL.md 文件
...
### 场景 1: 学习新技术
```bash
python src/index.py search "typescript"
python src/index.py install "typescript" 1
```

### 场景 2: 提高测试能力
```bash
python src/index.py search "testing"
# 查看结果，选择合适的编号
python src/index.py install "testing" 3
```

### 场景 3: DevOps 工作
```bash
python src/index.py search "docker compose"
python src/index.py install "docker compose" 1
```
...
**版本**: 2.0.0
**创建日期**: 2025-12-26 (Node.js)
**更新日期**: 2026-01-08 (Python)
**数据库版本**: 2025-12-26
